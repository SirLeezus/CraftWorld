...
public class PlayerMovement : MonoBehaviour
{
    ...
    {
        Vector3 horizontalMovement = GetMovementDirection(movementInput);
        Vector3 verticalMovement = Vector3.zero;

        if (ascendInput)
        {
            verticalMovement = Vector3.up * flySpeed;
        }
        else if (descendInput)
        {
            verticalMovement = Vector3.down * flySpeed;
        }

        Vector3 movementDirection = horizontalMovement + verticalMovement;
        movementDirection = movementDirection.normalized * flySpeed; // Normalize and set flySpeed
